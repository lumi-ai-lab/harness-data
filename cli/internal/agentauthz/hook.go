...
func missingAuthReason(dialect ShellDialect, command string, cfg harness.AuthzConfig, sourceErr error) string {
	...
	return "QDM_AUTHZ_SOURCE_MISSING: authz mode is on but no encrypted auth blob is bound with an explicit user ID; cannot run protected qdm-metric-cli data command"
}
