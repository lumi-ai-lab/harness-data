...
func TestAuthorizedCommandPinsMetricCLIAndPreservesOriginalCommand(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"export PATH='/runtime/with '\\'' quote/bin':\"${PATH:-}\"",
		"export QDM_METRIC_CLI='/runtime/with '\\'' quote/bin/qdm-metric-cli'",
...
func TestPreToolDeniesPrivateAndLegacyCLIs(t *testing.T) {
	root := t.TempDir()
	writeState(t, root, profileLumiRequired, "codex")
	...
	for name, command := range map[string]string{
		"private absolute path": "/opt/harness-data/private/qdm-metric-cli-v0.1.0 version",
		"private quoted path":   `"/opt/harness-data/private/qdm-metric-cli-v0.1.0" version`,
		"private runtime name":  "qdm-metric-cli-real version",
		"SQL CLI":               "qdm-sql-cli --help",
		"CMR CLI":               "qdm-cmr-cli --help",
		"CAS CLI":               "cas-cli --help",
	} {
		t.Run(name, func(t *testing.T) {
			input, err := json.Marshal(hookPayload{
				SessionID:     "session-a",
			...
			handled, value, err := run(root, "codex", input, binder)
			if err != nil || !handled {
				t.Fatalf("unexpected result: handled=%v err=%v", handled, err)
			}
			...
			if !ok || result.HookSpecificOutput == nil ||
				...
				t.Fatalf("response = %#v, want deny", value)
			}
		})
	}
}
...
func TestCodexProtectsCanonicalAndUnifiedExecToolIDs(t *testing.T) {
	for _, toolName := range []string{"Bash", "exec_command"} {
		if !isShellTool("codex", toolName) {
			t.Fatalf("Codex shell tool %q must be protected", toolName)
		}
	}
	if isShellTool("claude", "exec_command") {
		t.Fatal("Claude must keep its native Bash tool ID")
	}
}
...
func TestAuthorizedCommandDoesNotRunWhenEnvironmentSetupFails(t *testing.T) {
	...
	script := "readonly QDM_METRIC_CLI; " + authorizedCommand(
		"/runtime",
		"binding-value",
		"printf executed > "+shellQuote(marker),
	)
	process := exec.Command("sh", "-c", script)
	process.Env = append(os.Environ(), "QDM_METRIC_CLI=/stale/public metric CLI")
...
func TestAuthorizedCommandPreservesShellExitAndSanitizesEnvironment(t *testing.T) {
	...
	command := `printf 'binding=%s\nmetric-cli=%s\npath=%s\nforbidden=%s\n' "$HARNESS_AUTHZ_BINDING_V1" "$QDM_METRIC_CLI" "$PATH" "${QDM_CMR_CLI-unset}"; exit 7`
	...
	for _, expected := range []string{
		...
		"metric-cli=/runtime/bin/qdm-metric-cli",
		"path=/runtime/bin:",
		"forbidden=unset",
	} {
		if !strings.Contains(got, expected) {
...
func writeState(t *testing.T, root, profile, agent string) {
	...
	if profile == profileLumiRequired {
		tools = map[string]any{
			...
			"qdm-metric-cli":      map[string]any{},
			"qdm-metric-cli-real": map[string]any{},
		}
	}
