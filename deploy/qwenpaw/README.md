...
>   ```
>
>   两种接法在**原子性上没有区别**: 导出 CLI 是"新建临时文件 → 写完 → rename"(实测产物的出生时间
>   等于当天 08:00, 比 mtime 早几十毫秒 —— 原地截断写不会换 inode), 直接指过去也读不到半截文件。
>   选专用目录只是为了挂载面最小 + `session-hmac.secret` 不落到导出账号可写的地方。
>   更省事的变体: 把导出的 `-out` 直接指到专用密钥目录(再在导出后 `chmod 644`), 既零拷贝也没有无关文件。
>   注意 rename 会按创建者重新落权限, 所以**每天都要 chmod**, 一次改好不够。也别用 `chown` 把文件改走,
>   那会夺掉导出账号自己的写权限。
