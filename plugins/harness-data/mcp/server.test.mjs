...
import { workspaceIdentity } from "../../../packages/harness-runtime-node/src/root-context.mjs";

const serverPath = fileURLToPath(new URL("./server.mjs", import.meta.url));
const repoRoot = resolve(new URL("../../../", import.meta.url).pathname);
const pluginSkillPath = join(repoRoot, "plugins", "harness-data", "skills", "html-report", "SKILL.md");
const localSkillPath = join(repoRoot, ".codex", "skills", "html-report", "SKILL.md");
const mcpFixtureRoot = realpathSync.native(mkdtempSync(join(tmpdir(), "qdm-mcp-root-context-")));
const testDataRoot = join(mcpFixtureRoot, "data");
const testWorkspaceRoot = join(mcpFixtureRoot, "workspace");
const testConfigPath = join(testDataRoot, "config", "settings.json");
const testWorkspacePolicyPath = join(testDataRoot, "config", "workspace-policy.json");
const testContextFile = join(mcpFixtureRoot, "context.json");
mkdirSync(join(testDataRoot, "config"), { recursive: true });
mkdirSync(testWorkspaceRoot, { recursive: true });
writeFileSync(testConfigPath, `${JSON.stringify({ schemaVersion: 1, authz: { mode: "off" }, metricCliPath: "" }, null, 2)}\n`);
writeFileSync(testWorkspacePolicyPath, `${JSON.stringify({ schemaVersion: 1, mode: "allowlist", includeChildren: true, roots: [testWorkspaceRoot] }, null, 2)}\n`);
writeFileSync(testContextFile, `${JSON.stringify({
  schemaVersion: 1,
  host: "codex",
  pluginRoot: join(repoRoot, "plugins", "harness-data"),
  resourceRoot: repoRoot,
  dataRoot: testDataRoot,
  configPath: testConfigPath,
  workspacePolicyPath: testWorkspacePolicyPath,
}, null, 2)}\n`);

function allowWorkspace(workspaceRoot) {
  const policy = JSON.parse(readFileSync(testWorkspacePolicyPath, "utf8"));
  const canonical = realpathSync.native(resolve(workspaceRoot));
  policy.roots = [...new Set([...(policy.roots || []), canonical])];
  writeFileSync(testWorkspacePolicyPath, `${JSON.stringify(policy, null, 2)}\n`);
}

function stateRootFor(workspaceRoot = testWorkspaceRoot) {
  const canonicalWorkspace = existsSync(workspaceRoot) ? realpathSync.native(workspaceRoot) : resolve(workspaceRoot);
  return join(testDataRoot, "state", "workspaces", workspaceIdentity({ workspaceRoot: canonicalWorkspace, host: "codex", schemaVersion: 1 }));
}

function sessionDirFor(sessionId, workspaceRoot = testWorkspaceRoot) {
  return metricCliSessionDirFor(workspaceRoot, sessionId, stateRootFor(workspaceRoot));
}

function legacyHashedSessionDirFor(sessionId, workspaceRoot = testWorkspaceRoot) {
  return metricCliSessionDirFor(workspaceRoot, sessionId);
}

function legacySessionDirFor(sessionId, workspaceRoot = testWorkspaceRoot) {
...
function startServer(t, { env = process.env, cwd = repoRoot } = {}) {
  const serverEnv = {
    ...env,
    HARNESS_CONTEXT_FILE: env.HARNESS_CONTEXT_FILE || testContextFile,
    HARNESS_WORKSPACE_ROOT: env.HARNESS_WORKSPACE_ROOT || testWorkspaceRoot,
    PWD: env.PWD || testWorkspaceRoot,
  };
...
test("MCP blocks a disallowed workspace before creating a report session", async (t) => {
  const workspace = await mkdtemp(join(tmpdir(), "mcp-disallowed-workspace-"));
  t.after(async () => rm(workspace, { recursive: true, force: true }));
  const sessionId = `mcp-disallowed-${process.pid}-${Date.now()}`;
  ...
  const result = await callTool("html_report_start", { sessionId, userQuestion: "不应创建会话" });
  assert.match(result.error?.message || "", /QDM_WORKSPACE_NOT_ALLOWED/);
...
test("MCP uses the metric-cli UI session directory and recovers a pre-fix split session", async (t) => {
  ...
  const legacyHashed = legacyHashedSessionDirFor(sessionId);
  const legacy = legacySessionDirFor(sessionId);
  t.after(async () => {
    await rm(canonical, { recursive: true, force: true });
    await rm(legacyHashed, { recursive: true, force: true });
  ...
  await mkdir(join(legacyHashed, "debug"), { recursive: true });
  await writeFile(join(legacyHashed, "result.json"), `${JSON.stringify({
    status: "confirmed",
    cards: [card],
  }, null, 2)}\n`);
  await seedCachedCard(legacyHashed, card.id, [{ saleAmt: 100 }], { saleAmt: "销售额" });
  ...
  assert.equal(existsSync(join(canonical, "result.json")), true);
});

test("MCP rejects symlinks while migrating workspace-local sessions", async (t) => {
  ...
  const { rpc, callTool } = startServer(t);
  await rpc("initialize", {});
  const sessionId = `mcp-session-unsafe-${process.pid}-${Date.now()}`;
  const canonical = sessionDirFor(sessionId);
  const legacyHashed = legacyHashedSessionDirFor(sessionId);
  const legacyReadable = legacySessionDirFor(sessionId);
  const outside = join(mcpFixtureRoot, `outside-${sessionId}.json`);
  t.after(async () => {
    await rm(canonical, { recursive: true, force: true });
    await rm(legacyHashed, { recursive: true, force: true });
    await rm(legacyReadable, { recursive: true, force: true });
    await rm(outside, { force: true });
  });
  await mkdir(legacyHashed, { recursive: true });
  await writeFile(outside, "{}\n");
  await symlink(outside, join(legacyHashed, "result.json"));
  await mkdir(join(legacyReadable, "debug"), { recursive: true });
  await writeFile(join(legacyReadable, "debug", "mcp-pipeline-state.json"), "{}\n");

  const result = await callTool("html_report_status", { sessionId });
  assert.match(result.error?.message || "", /unsafe entry/);
  assert.equal(existsSync(canonical), false);
});

test("MCP rejects symlinked ancestors of workspace-local sessions", async (t) => {
  if (process.platform === "win32") {
    t.skip("symlink creation requires additional Windows privileges");
    return;
  }
  const workspace = await mkdtemp(join(tmpdir(), "mcp-workspace-symlink-"));
  const external = await mkdtemp(join(tmpdir(), "mcp-external-session-"));
  const sessionId = `mcp-session-ancestor-${process.pid}-${Date.now()}`;
  const externalHarness = join(external, ".harness");
  const externalHashed = legacyHashedSessionDirFor(sessionId, external);
  const externalReadable = legacySessionDirFor(sessionId, external);
  const canonical = sessionDirFor(sessionId, workspace);
  t.after(async () => {
    await rm(canonical, { recursive: true, force: true });
    await rm(workspace, { recursive: true, force: true });
    await rm(external, { recursive: true, force: true });
  });
  await mkdir(externalHashed, { recursive: true });
  await writeFile(join(externalHashed, "result.json"), "{}\n");
  await mkdir(join(externalReadable, "debug"), { recursive: true });
  await writeFile(join(externalReadable, "debug", "mcp-pipeline-state.json"), "{}\n");
  await symlink(externalHarness, join(workspace, ".harness"));
  allowWorkspace(workspace);

  const { rpc, callTool } = startServer(t, {
    env: { ...process.env, HARNESS_WORKSPACE_ROOT: workspace, PWD: workspace },
  });
  await rpc("initialize", {});
  const result = await callTool("html_report_status", { sessionId });
  assert.match(result.error?.message || "", /path contains a symlink/);
  assert.equal(existsSync(canonical), false);
});
...
test("B2_MAIN does not auto-export; generate_html works after explicit call", async (t) => {
  ...
  const missingConfirmation = await callTool("html_report_generate_html", { sessionId });
  assert.match(missingConfirmation.error?.message || "", /explicit user confirmation/);

  const generated = await callTool("html_report_generate_html", { sessionId, confirmation: "生成 HTML" });
  ...
  const again = await callTool("html_report_generate_html", { sessionId, confirmation: "生成 HTML" });
...
test("generate_html failure can be retried and rejects extra path arguments", async (t) => {
  ...
  const failed = await failing.callTool("html_report_generate_html", { sessionId, confirmation: "生成 HTML" });
  ...
  const retried = await retrying.callTool("html_report_generate_html", { sessionId, confirmation: "生成 HTML" });
