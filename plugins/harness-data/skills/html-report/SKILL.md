...
- **Never** call `html_report_generate_html` unless the user explicitly asked to generate HTML; pass the exact `confirmation: "生成 HTML"` value only after that reply.
...
- Runtime prerequisite: run the installed Plugin's `node scripts/setup.mjs` once to initialize `qdm-metric-cli`, Wikis/index, config, and authz outside the project directory.
