...
/**
 * Sort dimension codes by the same two-level priority as buildCaptionAxis
 * (group order in CAPTION_DIM_GROUPS, then dim index within each group),
 * but WITHOUT the caption caps — every requested dimension is kept.
 * Unknown codes are appended at the end in their original relative order.
 */
export function sortDimensionColumns(dimensions) {
  const requested = uniqueStrings(dimensions);
  const known = [];
  const unknown = [];
  for (const dim of requested) {
    const location = captionDimLocation(dim);
    if (location) known.push({ dim, ...location });
    else unknown.push(dim);
  }
  known.sort((a, b) =>
    a.groupIndex !== b.groupIndex
      ? a.groupIndex - b.groupIndex
      : a.dimIndex !== b.dimIndex
        ? a.dimIndex - b.dimIndex
        : 0
  );
  return [...known.map((item) => item.dim), ...unknown];
}
...
/**
 * Map a dimension code to its display-name column key, matching the CLI's
 * --dim-labels logic: *Id → *Name, otherwise append Name.
 */
export function dimensionLabelColumn(code) {
  const dim = String(code || "");
  if (dim.endsWith("Id")) return dim.slice(0, -2) + "Name";
  return dim + "Name";
}

/**
 * Resolve the actual column key present in a row for a dimension code.
 * With --dim-labels=only the ID column is replaced by a *Name column;
 * DATE and same-column dimensions keep their original code.
 * Falls back to the code when the label column is absent.
 */
export function resolveDimensionColumn(dim, row) {
  if (!row || typeof row !== "object" || Array.isArray(row)) return dim;
  const labelCol = dimensionLabelColumn(dim);
  if (labelCol !== dim && Object.prototype.hasOwnProperty.call(row, labelCol)) {
    return labelCol;
  }
  return dim;
}
