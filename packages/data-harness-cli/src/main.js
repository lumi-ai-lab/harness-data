...
export async function run(argv = process.argv.slice(2), io = process) {
  ...
  try {
    ...
    if (context) {
      // Legacy command handlers use resourceRoot (where wikis + config live).
      // Falls back to pluginRoot when resourceRoot is not set separately.
      root = context.resourceRoot || context.pluginRoot;
    } else {
