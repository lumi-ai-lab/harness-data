...
/**
 * Extract global Root Context options while preserving command-specific args.
 * Root flags are accepted before or after the command; a `--` terminator stops
 * global parsing after the command.
 */
...
export function loadRootContextFile(filePath) {
  ...
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw invalid("context file must contain a JSON object");
  }
  return parsed;
}

/**
 * Resolve explicit fields, a context file and host compatibility environment.
 * Explicit fields win over the file, which wins over environment variables.
 * Returns null when no structured context was supplied, allowing callers to
 * enter the legacy compatibility path explicitly.
 */
export function resolveRootContext({ contextFile = "", explicit = {}, env = process.env } = {}) {
  ...
  // A root override invalidates dependent paths unless they are explicitly
  // overridden too. This prevents stale locations from a context file from
  // surviving a data-root/workspace migration.
  if (explicit.dataRoot && explicit.configPath == null) delete merged.configPath;
  if ((explicit.dataRoot || explicit.workspaceRoot || explicit.host) && explicit.stateRoot == null) {
    delete merged.stateRoot;
  }
...
/**
 * Convert a host hook envelope into the same Root Context used by CLI calls.
 * Only host-declared fields are considered; a legacy caller with no structured
 * roots returns null so it can remain on the compatibility path.
 */
export function contextFromHookPayload(payload, { root = "", env = process.env, baseContext = null } = {}) {
  ...
  if (!explicit.dataRoot && env?.CODEX_HOME) explicit.dataRoot = env.CODEX_HOME;
  ...
  if (!explicit.workspaceRoot && (payload.workspaceRoot || payload.workspace_root)) {
    explicit.workspaceRoot = payload.workspaceRoot || payload.workspace_root;
  }
  const hasStructuredRoots = Boolean(explicit.pluginRoot || explicit.dataRoot || explicit.workspaceRoot || declared.pluginRoot || declared.dataRoot);
  ...
  if (!explicit.pluginRoot || !explicit.dataRoot) {
    throw invalid("hook envelope must declare pluginRoot and dataRoot");
  }
  if (explicit.capabilities && typeof explicit.capabilities === "object") {
    explicit.capabilities = { ...explicit.capabilities };
    if (explicit.workspaceRoot && payload.cwd && explicit.capabilities.canWriteWorkspace === false) {
      delete explicit.capabilities.canWriteWorkspace;
    }
    if (explicit.sessionId && (payload.session_id || payload.sessionId) && explicit.capabilities.hasStableSessionId === false) {
      delete explicit.capabilities.hasStableSessionId;
    }
  }
...
/**
 * Validate and normalize a Root Context v1 object. Existing paths are
 * realpath'ed (including symlinked parents); missing data/workspace paths are
 * retained as absolute paths so setup can create them later.
 */
export function normalizeRootContext(input, { source = "root context", requireWorkspace = false } = {}) {
  ...
  if (requireWorkspace && !workspaceRoot) {
  ...
  const configPath = normalizeRootPath(input.configPath, "configPath") || path.join(dataRoot, "config", "settings.json");
...
function contextFromEnv(env = process.env) {
  ...
  if (!values.dataRoot && env?.CODEX_HOME) values.dataRoot = String(env.CODEX_HOME);
  ...
  return {
    values,
    contextFile,
    hasStructuredOptions: Boolean(values.pluginRoot || values.dataRoot || contextFile),
  };
}
...
function normalizeSecretRef(value) {
  ...
  if (kind === "file") {
    ...
    const filePath = normalizeRootPath(ref.path, "secretRef.path", { required: true });
...
function validateSecretFileRef(value) {
  ...
  if (process.platform !== "win32" && (info.mode & 0o777) !== 0o600) {
    throw invalid("secretRef.path permissions must be 0600");
  }
}
...
function normalizeRootPath(value, name, options = {}) {
  if (value == null || String(value).trim() === "") {
    if (options.required) {
      throw new RootContextError(options.errorCode || ROOT_CONTEXT_ERROR_CODES.INVALID, `${name} is required`);
    }
    return "";
  }
  const text = String(value).trim();
  if (text.includes("\0") || !path.isAbsolute(text)) {
  ...
  if (existsSync(normalized) && (name === "configPath" || name === "secretRef.path") && isDirectory(normalized)) {
...
function validateRootRelationships({ pluginRoot, dataRoot, secretRoot, workspaceRoot, stateRoot, configPath, secretRef }) {
  const primary = [
  ...
  for (let i = 0; i < primary.length; i += 1) {
    for (let j = i + 1; j < primary.length; j += 1) {
      const [leftName, left] = primary[i];
      const [rightName, right] = primary[j];
      if (isPathWithin(left, right) || isPathWithin(right, left)) {
        throw invalid(`${leftName} and ${rightName} overlap`);
      }
    }
  }
  if (stateRoot && !isPathWithin(dataRoot, stateRoot)) throw invalid("stateRoot must be inside dataRoot");
  if (configPath && !isPathWithin(dataRoot, configPath)) throw invalid("configPath must be inside dataRoot");
...
function readFlagValue(argv, index, flag, scope) {
  if (flag.inline != null) return { value: flag.inline, nextIndex: index };
  const next = argv[index + 1];
  if (next == null || String(next).startsWith("--")) {
    throw invalid(`flag --${flag.name} needs an argument (${scope})`);
  }
...
function parseLongFlag(token) {
  ...
  return equals >= 0
    ? { name: raw.slice(0, equals), inline: raw.slice(equals + 1) }
    : { name: raw, inline: null };
}
