...
六个模块全部成功取数后，下一步必须立即执行该 template 注入命令；template 注入前禁止总结、禁止整理报告素材、禁止生成中间分析、禁止输出阶段性结论。
...
- `bin/data-harness-cli inject-template` 不并行，必须在并行查询整体成功后的下一步立即执行。
- 并行查询整体成功后、template 注入前，不得先整理证据、总结素材、生成中间分析或输出阶段性结论。
