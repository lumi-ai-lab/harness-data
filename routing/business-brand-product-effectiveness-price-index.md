---
id: routing-business-brand-product-effectiveness-price-index
kind: routing
domain: business
title: 经营分析-售价价格指数指标路由规则
tags:
  - routing
  - business-report
...
判断为售价价格指数专项报告时，固定走 `qdm-cmr-cli report business`，固定 `--indicator 售价价格指数(线上)`。
...
- 对 CLI 未返回的数据，不得自行估算。
- 不得路由到品效、采购价格指数、商品订购渗透率或定价毛利率专项；泛经营问题继续走 `routing/business-overview.md`。
