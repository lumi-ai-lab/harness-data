---
id: routing-business-cust-penetration-rate-per-cust-amt
kind: routing
domain: business
title: 经营分析-客单价指标路由规则
tags:
  - routing
  - business-report
...
判断为客单价专项报告时，固定走 `qdm-cmr-cli report business`，固定 `--indicator 客单价`。
...
- 对 CLI 未返回的数据，不得自行估算。
- 不得路由到客数渗透率根指标、19点前客单价、销售额、客数、品效或活跃供应商数专项；泛经营问题继续走 `routing/business-overview.md`。
