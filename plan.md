# Harness Data Plugin-first 实施方案

状态：Codex Plugin 主链路实施中
最近更新：2026-08-31
当前产品决策：主 GitHub 仓库直接作为 Marketplace；正式 Codex Plugin 身份为 `harness-data@lumi-ai-lab`；Wiki 内置于 Plugin；Setup 允许写入 Plugin 管理目录；普通状态和报告 Pipeline 状态写入 Plugin 外的 dataRoot；最终 Markdown 写入 `workspaceRoot/analysis/main.md`。

本文件后续较早章节保留历史设计和验证记录。若历史章节与本节冲突，以本节和 `docs/codex-plugin-layout.md` 为准。

## 2026-08-31 当前方案

```text
lumi-ai-lab/harness-data (主 GitHub 仓库)
└── Marketplace: lumi-ai-lab
    └── Plugin: harness-data
        ├── shared core + Agent adapters
        ├── resources/wikis/ (唯一一份内置 Wiki)
        ├── dist/ (Plugin 自带运行时代码)
        └── scripts/setup.mjs

Setup 写入 Plugin:
  context.json / config / secrets / runtimes / .harness/index / manifests

Setup 和运行时写入 Plugin 外:
  dataRoot/state/workspaces/<workspace-hash>/

最终报告:
  workspaceRoot/analysis/main.md
  workspaceRoot/analysis/main.html (用户明确确认后)
```

当前不作为用户发布渠道维护 Marketplace 独立仓库、Marketplace ZIP、Harness Data runtime/Wiki ZIP、GHCR、通用 host artifact 或独立 npm 安装入口。`qdm-metric-cli` 由独立仓库发布，Plugin Setup 按平台动态从 Gitee latest 回退 GitHub latest 获取。

Hermes 和 OpenClaw 适配层、测试、配置及产品引用已从当前产品范围移除。其他 Agent 适配代码可以继续在同一主仓库维护，但不改变 Codex Marketplace 的唯一安装入口。

## 历史方案记录

以下章节记录迁移前的多宿主双根设计、兼容策略和历史发布验证，供内部追溯使用。
...
- plugins/harness-data/scripts/bundle-dist.mjs:12
...
- 本轮实施补强：`update` 通过 install session 覆盖 runtime、CLI、Wikis、hooks、config 和 installer state 的全流程回滚；新增 `CODEX_HOME`/`HARNESS_DATA_ROOT` 优先级、`CODEX_WORKSPACE_ROOT` identity、host secretRef 脱敏、继承 FD secret transport 和 host artifact `bin/scripts/data-harness-cli` wrapper 回归。
- 本轮回归结果：`npm test -- --test-concurrency=1` 为 181 pass；`packages/data-harness-cli` 为 82 pass + 2 skip；`packages/harness-runtime-node` 为 18 pass；host artifact/verifier 为 5 pass；`npm pack --dry-run`、`npm run verify:artifact` 和 `git diff --check` 均通过。
- Claude Phase 5 已补齐原生 `.claude-plugin/plugin.json`、标准 `hooks/hooks.json`，并新增 `build-claude-marketplace.mjs` 生成包含 `${CLAUDE_PLUGIN_ROOT}/scripts/data-harness-cli` wrapper 与 vendor core 的可安装 marketplace；源 manifest、生成 marketplace 的 `claude plugin validate`、七宿主 artifact matrix 与 Claude hook no-op smoke 通过，真实 Claude 模型会话、生产 secret provider 和桌面 reload 仍待实机；CLI install/enable/disable/enable 及 `0.0.54 → 0.0.55-test → 0.0.54` update/rollback 已完成，详见 `docs/claude-phase5.md`。
...
- Codex CLI TUI 实机 smoke 已完成：真实 remove/add 重装后，在新会话中确认 `harness-data` 已安装启用、`html-report` MCP 为 connected（6 tools），且 `$html-report` skill 可被发现；缓存 MCP self-test 为 10/10。证据见 `docs/codex-golden-path.md`。
...
- 本轮补齐 Claude 原生 marketplace 构建链路：`scripts/build-claude-marketplace.mjs` 生成可直接安装的 `.claude-plugin/marketplace.json`、原生 plugin manifest、hooks、wrapper 和 vendor core；真实 Claude CLI `2.1.251` 已完成本地 marketplace discovery/install/enable/disable/enable、普通 hook no-op 及 `0.0.54 → 0.0.55-test → 0.0.54` update/rollback smoke，临时插件验证后已清理。真实 Claude 模型会话、生产 secret provider、桌面 reload 仍待补。
- 本轮补强 runtime artifact：`scripts/build-runtime-artifact.sh` 现在同时产出 `agents/claude-marketplace`，并在 runtime artifact 校验中确认其原生安装入口存在；新增 builder 单测与 CI/release 校验入口。
- 本轮补充 Pi clean-profile 实机证据：Pi CLI `0.84.3` 完成本地插件 install/list/remove，隔离 profile 清理后无残留；由于 profile 无 provider API key，真实 session/reload、workspace/secret handoff 和报告流程仍待补，详见 `docs/pi-phase5.md`。
- 本轮补强宿主适配层：Pi authz fixture 显式保持 `0600` 并回归 80 pass；OpenClaw 与 Hermes 不再从 `PWD` 或插件目录向上猜测 workspace，改为接受结构化 root context/宿主环境变量，缺少显式 workspace 时安全 no-op/失败关闭。真实宿主 discovery、secret、session 和 reload 证据仍待补。
- 本轮最终回归：npm installer 181 pass；data-harness-cli 82 pass + 2 skip；harness-runtime-node 18 pass；artifact/golden-path 定向回归 25 pass；`0.0.54` 与 `0.0.55-test` 两套 runtime artifact 均 build/verify 成功且 MCP self-test 10/10；`npm pack --dry-run`、`npm run verify:artifact` 和 `git diff --check` 均通过。
- Codex Phase 5 版本切换补证（2026-08-29）：真实 Codex CLI `0.150.1` 已完成 `0.0.54 → 0.0.55-test → 0.0.54` marketplace/plugin 切换；两个版本均 installed+enabled、MCP 指向对应缓存、self-test 10/10、doctor PASS。显式 dataRoot、metric-cli、workspace identity、0600 file secretRef 和 `A_CONFIG` report session 在升级/回滚后保持可用。验收同时修复 native/product manifest 版本不一致、Codex marketplace 缺少 runtime manifest，以及 setup/doctor 沿用旧 plugin version 的问题；本机最终稳定安装为 `0.0.54`，marketplace 已迁移到非临时的 Codex local-marketplaces 目录。证据见 `docs/codex-phase5-version-switch-2026-08-29.md`。
- Codex/Claude 宿主验收变更已提交为 `8219400`（`fix(codex): 补齐版本切换与持久性验收`）并进入 `harness-data#77`；PR 当前保持 OPEN。远端 Verify Host Artifacts run `33275463035`、Wikis Compatibility run `33275463049`、Publish CLI Container run `33275462995` 全部通过，Linux/macOS/Windows migration jobs 均为 SUCCESS；Publish GitHub Release 在普通 PR 上按条件 SKIPPED，未冒充正式发布证据。
- 当前仍未完成：Codex 桌面客户端 reload/new-session 因 Computer Use 安全策略拒绝控制 `com.openai.codex`，P5-02 因此保持未完成；真实版本切换、显式 dataRoot、workspace/file-secret handoff、卸载重装、升级回滚和 session 恢复已补齐。宿主原生 secret API 不再作为当前版本前置条件，生产通道确定为 0600 file `secretRef` 或继承 FD。其余宿主的 Phase 5/6 实机验收也未完成。
- 下一步按依赖顺序：由人工补 Codex 桌面 reload/new-session；再为 Claude 补登录后的模型会话、生产 secret 和 reload；随后完成 WorkBuddy、Pi（provider/session 尚缺）、OpenClaw、Hermes、QwenPaw 的 Phase 5 实机证据，最后执行 Phase 6 清理和稳定化。
- 本次进度更新（2026-08-29）：已将 0600 file `secretRef`/继承 FD 确认为当前生产 secret 通道，并新增 `docs/support-runbook.md` 与 `docs/codex-desktop-reload-checklist.md`；Pi authz fixture 修复为显式 0600，扩展测试 80 pass；OpenClaw/Hermes adapter 已改为显式 Root Context/宿主环境变量，缺少 workspace 时不再从 `PWD` 或插件目录猜测；新增 OpenClaw 3 项、Hermes 3 项测试并通过。Codex Desktop、Claude 登录态和其余宿主真实生命周期证据仍待补，Phase 6 继续保持未开始。
- 本轮按 Phase 5 顺序执行（2026-08-29）：Computer Use 尝试读取真实 Codex Desktop（`com.openai.codex`）被桌面安全策略拒绝，P5-02 仍需人工接管；Claude CLI `2.1.251` 已完成本地 marketplace 构建、校验、install/enable smoke 和清理，但 `claude auth status` 显示未登录，真实模型会话、secret handoff、报告和 reload 仍无法验收；Pi clean profile install/list/remove、Pi authz 80 项、OpenClaw 3 项、Hermes 3 项测试，以及七宿主 artifact build/verify/self-test 均通过。正确入口回归为 npm 181 pass、CLI 82 pass + 2 skip、runtime 18 pass；这些结果不替代真实宿主 Phase 5 证据。
- 当前执行结论：P5-01、P5-02、P5-03、P5-04、P5-05、P5-06、P5-07 的真实宿主验收项均保持未勾选；当前只完成可自动化的构建、适配层和 clean-profile 证据。未获得 Codex Desktop 人工结果、Claude 登录态、Pi provider 或其他宿主实机环境前，不推进 Phase 6，不删除旧 `install --dir`。
...
- [x] P0-12（文档）补齐或归档本方案引用的 handoff 调研文档；原始文件无法从工作树或 Git 对象恢复，已用当前代码/发布链路重新留证，见 `docs/handoff-plugin-first-directory-layout-2026-08-29.md`。
...
最小验证：`cd npm && npm test -- --test-concurrency=1`；宿主 clean-install/setup/doctor/reload fixture；html-report 运行 `node plugins/harness-data/mcp/server.mjs --self-test`（若该宿主使用该 MCP）。
...
- [x] 完成 `CODEX_HOME`/显式 dataRoot、workspace/file-secret handoff、卸载重装、升级回滚 smoke；真实 `0.0.54 → 0.0.55-test → 0.0.54` 切换、doctor 和 report session 恢复已通过。生产 secret 通道确定为 0600 file `secretRef` 或继承 FD。
- [ ] 完成 Codex Desktop reload/new-session；当前自动化控制受安全策略限制，需人工在真实桌面客户端验收。
- [x] 记录 artifact、版本、能力和环境限制证据到 capability matrix/runbook，见 `docs/codex-golden-path.md` 和 `docs/codex-phase5-version-switch-2026-08-29.md`；最新远端证据为 `harness-data#77` / commit `8219400`。

#### P5-03 WorkBuddy

- [ ] 通过实机确认 plugin data、workspace 和 session API；仓库内 adapter/单测已具备，实机确认后再补宿主特有映射。
...
- [ ] 完成 workspace/secret handoff、clean profile、卸载重装、升级回滚 smoke；隔离 profile 的 install/list/remove 与 authz 扩展 80 pass 已通过，真实 provider/session 和升级回滚仍待补。
- [x] 记录 `build`/`verify`/`test` 输出及 artifact 证据，见 `docs/pi-phase5.md`。

#### P5-05 OpenClaw

- [ ] 确认宿主 state 目录与 session 能力，完成 discovery/install/enablement/hook；adapter 已支持显式 Root Context，真实宿主仍待验证。
- [ ] 完成 workspace/secret handoff、卸载重装、升级回滚 smoke。
- [ ] 记录 artifact、版本、能力和失败证据到 capability matrix/runbook。

#### P5-06 Hermes

- [ ] 确认宿主 plugin data 目录与 secret API，完成 discovery/install/enablement/hook；Python/shell adapter 已支持显式 Root Context，真实宿主仍待验证。
- [ ] 完成 workspace/secret handoff、卸载重装、升级回滚 smoke。
- [ ] 记录 artifact、版本、能力和失败证据到 capability matrix/runbook。
...
- [ ] P6-03 同步文档、doctor、错误码、迁移指引和 support runbook；support runbook 初稿已新增，最终同步待发布周期完成。
- [ ] P6-04 完成至少一个完整发布周期的回归观察，确认无阻塞性回归；当前仅有正式 `0.0.54`，尚无正式 `0.0.55` 周期证据。
...
- html-report self-test：`node plugins/harness-data/mcp/server.mjs --self-test`。
- Pi artifact：`npm --prefix plugins/pi-html-report run build`、`run verify`、`npm test`。
- Pi adapter/authz：`node --test .agents/pi/extensions/qdm-harness/test/*.test.mjs`。
- OpenClaw adapter：`node --test .agents/openclaw/plugins/qdm-harness/test/index.test.mjs`。
- Hermes adapter：`python3 -m unittest discover -s .agents/hermes/plugins/qdm-harness/test -p 'test_*.py' -v`。
