# Lumi × Harness Data：Pi / Indicators CLI 请求级权限受控试点方案
...
3. Harness 仅适配 Pi。Claude Code、Codex、Qwen 及其他 Agent 不在首发范围内。
4. 公开的 `bin/qdm-indicators-cli` 改为 Facade；真实 v0.0.4 CLI 使用固定私有绝对路径执行，不进入 Agent 的 `PATH`。
5. 每个 Pi Bash tool call 都携带当前请求的私有绑定；Facade 每次启动都重新读取 Lumi 的精确 Session 文件并核验绑定，不能只信任提示词、进程缓存或环境变量。
...
- Pi 请求级绑定、脱敏提示摘要和 Bash child 环境传播。
...
- 当前 Harness prompt hook 只读取 `session_id` 和 `prompt`，不会自动消费 ACP `_meta`。
- Pi 的 `context` 事件会随当前用户消息构造模型上下文；`before_agent_start` 只适合静态说明，不能用于每条消息换绑。
- 当前 Pi `tool_call` 已能检查并改写 Bash tool input，可在该位置给整个 Bash child 执行域注入绑定。
...
企业微信消息
  ...
  -> Pi context 事件使用精确 sessionId 文件名读取 envelope
  -> 严格校验 envelope
  -> 在 Pi 扩展私有状态中创建 HarnessAuthzBinding V1
  -> 仅向模型注入脱敏的身份/范围摘要
  -> Pi tool_call 给本次 Bash child 注入 binding 环境
...
| Pi `context` adapter | 取得当前 Pi session ID；读取精确文件；严格校验；创建/更新私有 binding；向模型提供脱敏摘要 | 不从自然语言推断用户；不扫描上下文目录 |
| Pi `tool_call` adapter | 按当前 session 取私有 binding；将其注入每个 Bash child，使脚本和子进程继承 | 不把绑定当作可信授权结论 |
...
### 5.5 Pi Session ID 前置验证

实施前必须先在真实 Lumi Sandbox 跑 smoke test，证明：

```text
Pi ctx.sessionManager.getSessionId() === ACP session/prompt.params.sessionId
```

测试必须覆盖新 Session、恢复 Session、同一 Session 的第二条入站消息和同一消息的自动 continuation。

如果不相等，必须修改 Lumi 到 Pi 的 ACP adapter，显式把 ACP session ID 传给 Pi 扩展；在该能力完成前，`lumi-mvp-required` readiness 失败。禁止用扫描 context 目录、取最新文件或猜测映射作为 fallback。

### 5.6 模型可见摘要

Pi `context` 事件可追加如下语义摘要：
...
首发固定并验证 Pi `0.81.1` 的扩展合同。虽然 `tool_call` input 本身只有 `command/timeout`，该版本支持通过 `createBashTool(..., spawnHook)` 注册同名的 auth-aware Bash tool，并在真正 spawn child 时提供环境。不得用 shell 字符串拼接 `export ...; original-command` 模拟环境注入。
...
- build-time 执行 `harness-data install --profile lumi-mvp-required --agent pi`，只组装并验证 Facade、真实 CLI、catalog、Pi adapter、schema/template 和完整 release-set；它记录运行时 authz config 的约定路径，但不读取或要求 workspace/agent 专属 context 目录及 credential secret。
...
- Lumi 升级以完整新 image digest 为单位；构建/验证任一步失败不发布。回滚就是重新部署上一完整 image digest，因此 Facade、真实 CLI、catalog、配置模板和 Pi 合同一起恢复。
...
| Lumi Sandbox | 在 Docker image 构建期选择 `lumi-mvp-required` 并打包 catalog；运行时只读挂载实际 authz config、host control 目录和运维凭证；固定 Pi 0.81.1 |
...
1. 在真实 Lumi Sandbox 验证 Pi native session ID 与 ACP session ID 完全一致。
2. 若不一致，先在 Lumi/Pi ACP adapter 中显式传播 ACP ID；不提供目录扫描 fallback。
3. 固定 Pi 0.81.1，并用真实 `createBashTool/spawnHook` smoke test 证明 child env 注入合同。
...
- [ ] 真实 Sandbox 证明 Pi native session ID 等于 ACP session ID；不一致时 readiness 失败。
...
1. Pi/ACP session ID smoke test 通过，或已完成显式 ACP ID 传播；Pi 0.81.1 `spawnHook` 合同已通过真实 child-env 验证。
