---
id: business-cust-penetration-rate-scm-store-profit-notax-rate
kind: spec
domain: business
title: 供应链毛利率指标报告规则
tags:
...
该文件用于经营分析下客数渗透率树中的供应链毛利率专项报告。取数固定使用 `qdm-cmr-cli report business`，指标固定为 `供应链毛利率` 或 `scmStoreProfitNotaxRate`。

## 指标位置

- 当前指标 code：`scmStoreProfitNotaxRate`
- 当前指标中文名：供应链毛利率
