...
test("setup surfaces a migration hint but does not migrate automatically", async (t) => {
  const f = fixture();
  t.after(() => fs.rmSync(f.root, { recursive: true, force: true }));
  fs.cpSync(path.join(path.dirname(new URL(import.meta.url).pathname), "..", "..", "packages", "data-harness-cli"), path.join(f.plugin, "packages", "data-harness-cli"), { recursive: true });
  const metricCli = write(f.plugin, "bin/qdm-metric-cli", "#!/bin/sh\nexit 0\n");
  ...
  const report = await setupCommand({
    ...
    legacyRuntime: f.legacy,
    metricCli,
    wikisSource: path.join(f.legacy, "wikis"),
    noAuth: true,
    json: true,
  }, capture({ CODEX_HOME: "" }));
  assert.equal(report.migration.status, "available");
