  ...
  try {
    ...
    if (dataRoot && workspaceRoot) {
      ...
      report.items.push(
        ...
        item("workspace-policy", "", path.join(dataRoot, "config", "workspace-policy.json"), "write", {}),
...
async function executeMigration(plan) {
  ...
  try {
    ...
    const settings = {
      ...
      workspacePolicyPath: path.join(target.dataRoot, "config", "workspace-policy.json"),
    ...
    const workspacePolicy = {
      ...
      roots: [target.workspaceRoot],
      updatedAt: new Date().toISOString(),
    };
    const workspacePolicyTarget = path.join(stagedDataRoot, "config", "workspace-policy.json");
    writeJsonAtomic(workspacePolicyTarget, workspacePolicy, 0o600);
    records.push({
      name: "workspace-policy",
      target: path.join(target.dataRoot, "config", "workspace-policy.json"),
      sha256: sha256File(workspacePolicyTarget),
      status: "written",
    });
    ...
    const doctor = await collectRootDoctor(normalizeRootContext({
      schemaVersion: 1,
      host: target.host,
      // The legacy runtime remains the read-only compatibility plugin root.
      // Migrated config, manifest, resource copy, and metric binary belong to
      // the new data-root layout and must be checked there explicitly.
      pluginRoot: target.pluginRoot || source.root,
      resourceRoot: path.join(target.dataRoot, "resources", "legacy", source.id),
      dataRoot: target.dataRoot,
      secretRoot: auth.secret ? target.secretRoot : "",
      workspaceRoot: target.workspaceRoot,
      stateRoot: target.stateRoot,
      configPath: path.join(target.dataRoot, "config", "settings.json"),
      workspacePolicyPath: path.join(target.dataRoot, "config", "workspace-policy.json"),
...
function verifyCompletedMigration({ source, target, items, auth, pointer }) {
  try {
    ...
    if (settings.dataRoot !== target.dataRoot
      ...
      || settings.workspacePolicyPath !== path.join(target.dataRoot, "config", "workspace-policy.json")
    ...
    if (install.dataRoot !== target.dataRoot
      ...
      || install.workspacePolicyPath !== path.join(target.dataRoot, "config", "workspace-policy.json")
