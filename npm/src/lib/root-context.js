...
export function resolveRootContext(options = {}, { env = process.env, requirePluginRoot = true } = {}) {
  ...
  if (!merged.dataRoot && env?.CODEX_HOME) merged.dataRoot = String(env.CODEX_HOME);
...
export function normalizeRootContext(input, { source = "root context", requirePluginRoot = true } = {}) {
  ...
  const configPath = normalizeFilePath(input.configPath, "configPath") || path.join(dataRoot, "config", "settings.json");
  const secretRef = normalizeSecretRef(input.secretRef);
  const sessionId = stringValue(input.sessionId);

  validateRootRelationships({ pluginRoot, dataRoot, secretRoot, workspaceRoot, stateRoot, configPath, secretRef });
...
export function defaultDataRoot(_host = "unknown") {
  if (process.platform === "darwin") return path.join(os.homedir(), "Library", "Application Support", "lumi-ai-lab", "qdm-harness", "data");
  if (process.platform === "win32") return path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "lumi-ai-lab", "qdm-harness", "data");
  return path.join(process.env.XDG_DATA_HOME || path.join(os.homedir(), ".local", "share"), "lumi-ai-lab", "qdm-harness", "data");
}

export function defaultSecretRoot(_host = "unknown") {
  if (process.platform === "darwin") return path.join(os.homedir(), "Library", "Application Support", "lumi-ai-lab", "qdm-harness", "secrets");
  if (process.platform === "win32") return path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "lumi-ai-lab", "qdm-harness", "secrets");
  return path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), ".config"), "lumi-ai-lab", "qdm-harness", "secrets");
}
...
function validateRootRelationships({ pluginRoot, dataRoot, secretRoot, workspaceRoot, stateRoot, configPath, secretRef }) {
  const primary = [["pluginRoot", pluginRoot], ["dataRoot", dataRoot], ["secretRoot", secretRoot], ["workspaceRoot", workspaceRoot]].filter(([, value]) => value);
  ...
  if (!isPathWithin(dataRoot, configPath)) throw invalid("configPath must be inside dataRoot");
