...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取销售额（`saleAmt`）指标的值、趋势、区域表现和品类表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code saleAmt --full
```

返回字段：`indicatorsName`（销售额）、`businessDefinition`（门店销售商品的总金额）、`statisticalLogic`（原价销售额减去日结相关优惠金额后，剩余的结算金额）、`indicatorBiz`（销售经营）。
...
```bash
qdm-cmr-cli report business indicators --indicator saleAmt --display-mode yoyMom
```
...
```json
{
  ...
  "threshold": {
    "compareSymbol": "GE",
    "compareValue1": 14720.47,
    "compareValue2": 14720.47,
    "compareValueType": 1
  }
}
```
...
- `valueUnit: 2` — 百分比/比率/金额（直接用值，如 19316.37 表示销售额为 19316.37 元）
...
```bash
qdm-cmr-cli report business trend --indicator saleAmt
```

返回最近约 30 天的逐日销售额数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report business area --indicator saleAmt
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的销售额排名（含同比、环比）：
...
```bash
qdm-cmr-cli report business category --indicator saleAmt
```

默认按**大分类**（`categoryLevel1Id`）维度分组，返回全品类销售额排名（含同比、环比）：
...
- 常见 `--category-type` 映射：
  - `大分类` → `categoryLevel1Id`（如 10 蔬菜、13 猪肉、12 水果、11 水产、24 肉禽蛋）
  - `中分类` → `categoryLevel2Id`（如 1124 两栖类）
  - `小分类` → `categoryLevel3Id`
  - `商品` → `articleId`
- 品类过滤与下钻：当指定大分类后，`area` 和 `category` 子命令会在此品类范围内进一步下钻。例如 `--category-type 大分类 --category 10` 时，`category` 子命令会显示蔬菜大分类下各中分类的销售额表现。
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -0.0533` 表示箭头向上但数值为 -5.33%（即环比下降）。
