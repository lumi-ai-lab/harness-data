# vip1活跃会员数取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取 vip1活跃会员数（`vip1ActiveMemberNum`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code vip1ActiveMemberNum --full
```

返回字段：`indicatorsName`（vip1活跃会员数）、`businessDefinition`（会员等级为VIP1的活跃会员）、`statisticalLogic`（会员等级为VIP1的活跃会员id去重计数）。
...
```json
{
  "indicatorCode": "vip1ActiveMemberNum",
  "indicatorName": "vip1活跃会员数",
  "value": 121.51,
  "valueUnit": 1,
  "zhCNUnit": "万",
  "mom": {
...
```bash
qdm-cmr-cli report user trend --indicator vip1ActiveMemberNum
```

返回最近约 30 天的逐日 vip1活跃会员数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report user area --indicator vip1ActiveMemberNum
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的 vip1活跃会员数排名（含同比、环比）：
...
- 常见 `--area-type` 映射：
  - `管理区域` -> `manageAreaId`（如 CN00 全国、CN01 粤西、CN18 粤东、CN15 华东）
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -0.0069` 表示箭头向上但数值为负（即下降）。
...
7. vip1活跃会员数的单位为"万"，`value: 121.51` 表示 121.51 万人。