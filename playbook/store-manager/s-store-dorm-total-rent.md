# 租金和物业管理费指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取租金和物业管理费（`storeDormTotalRent`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code storeDormTotalRent --full
```

返回字段：`indicatorsName`（租金和物业管理费）、`businessDefinition`（门店租金及物业管理费用合计（万元））、`statisticalLogic`（统计周期内门店租金与物业管理费合计）、`indicatorBiz`（门店管理）。
...
```bash
qdm-cmr-cli report store indicators --indicator storeDormTotalRent --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "storeDormTotalRent",
  "indicatorName": "租金和物业管理费",
  "value": 461.88,
  "valueUnit": 2,
  "mom": {
    "value": 0,
...
- `valueUnit: 2` -- 百分比/比率/金额（直接用值，如 461.88 表示租金和物业管理费为 461.88 万元）
...
```bash
qdm-cmr-cli report store trend --indicator storeDormTotalRent
```

返回最近约 30 天的逐日租金和物业管理费数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator storeDormTotalRent
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的租金和物业管理费排名（含同比、环比）：
