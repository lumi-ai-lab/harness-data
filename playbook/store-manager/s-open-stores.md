...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取开店数（`openStores`）指标的值、趋势、区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code openStores --full
```

返回字段：`indicatorsName`（开店数）、`businessDefinition`（报告期内完成开业流程并正式营业的门店数量）、`statisticalLogic`（统计报告期内状态由"待开业"或"筹建中"转为"营业中"的门店数量）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator openStores --display-mode yoyMom
```
...
```bash
qdm-cmr-cli report store trend --indicator openStores
```

返回最近约 30 天的逐日开店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator openStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的开店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -9` 表示箭头向上但数值为 -9（即环比减少 9 家门店）。
