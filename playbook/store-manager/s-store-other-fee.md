# 门店其他支出指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取门店其他支出（`storeOtherFee`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code storeOtherFee --full
```

返回字段：`indicatorsName`（门店其他支出）、`businessDefinition`（门店除工资、租金、水电外的其他运营支出合计（万元））、`statisticalLogic`（统计周期内门店其他运营支出合计）、`indicatorBiz`（门店管理）。
...
```bash
qdm-cmr-cli report store indicators --indicator storeOtherFee --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "storeOtherFee",
  "indicatorName": "门店其他支出",
  "value": 343.32,
  "valueUnit": 2,
  "mom": {
    "value": 0,
...
- `valueUnit: 2` -- 百分比/比率/金额（直接用值，如 343.32 表示门店其他支出为 343.32 万元）
...
```bash
qdm-cmr-cli report store trend --indicator storeOtherFee
```

返回最近约 30 天的逐日门店其他支出数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator storeOtherFee
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的门店其他支出排名（含同比、环比）：
