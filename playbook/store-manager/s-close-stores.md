...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取闭店数（`closeStores`）指标的值、趋势、区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code closeStores --full
```

返回字段：`indicatorsName`（闭店数）、`businessDefinition`（报告期内完成闭店流程的门店数量）、`statisticalLogic`（统计报告期内状态由"营业中"或"停业"转为"已关闭"的门店数量）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator closeStores --display-mode yoyMom
```
...
```bash
qdm-cmr-cli report store trend --indicator closeStores
```

返回最近约 30 天的逐日闭店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator closeStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的闭店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "down"` 且 `value: 1` 表示箭头向下但数值为 +1（即闭店数增加 1 家，为负面信号）。
