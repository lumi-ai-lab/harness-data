...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取停业门店数（`stopBusinessStores`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code stopBusinessStores --full
```

返回字段：`indicatorsName`（停业门店数）、`businessDefinition`（当前处于停业状态的门店数量）、`statisticalLogic`（统计当前处于停业状态的门店总数）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator stopBusinessStores --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "stopBusinessStores",
  "indicatorName": "停业门店数",
  "value": 19,
  "valueUnit": 1,
  "mom": {
...
- `valueUnit: 1` — 整数值（如门店数，19 表示 19 家门店）
- `valueUnit: 2` — 百分比/比率/金额（直接用值）
- `valueUnit: 3` — 小数形式的比率（需 ×100 转为百分比）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量（如 `-5` 表示同比减少 5 家门店）
...
```bash
qdm-cmr-cli report store trend --indicator stopBusinessStores
```

返回最近约 30 天的逐日停业门店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator stopBusinessStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的停业门店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "down"` 且 `value: -5` 表示箭头向下、同比减少 5 家门店。
