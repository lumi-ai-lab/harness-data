# 头部门店不良率指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取头部门店不良率（`headLossRate`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。
> 品类口径固定为全品类，不支持品类下钻。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code headLossRate --full
```

返回字段：`indicatorsName`（头部门店不良率）、`businessDefinition`（头部门店中不良门店的占比）、`indicatorBiz`（门店盈利与运营效率）。
...
```bash
qdm-cmr-cli report store indicators --indicator headLossRate --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "headLossRate",
  "indicatorName": "头部门店不良率",
  "value": 0.0520,            // 当前值（valueUnit=3，需×100=5.20%）
  "valueUnit": 3,              // 单位类型（3=小数形式的比率，需×100转为百分比）
  "mom": {
    "value": 0.0003,          // 环比变动（+0.03个百分点）
    "status": "up",
    "unit": 3
  },
  "yoy": {
    "value": -0.0015,         // 同比变动（-0.15个百分点）
    "status": "down",
    "unit": 3
  }
}
...
- `valueUnit: 3` — 小数形式的比率（需 ×100 转为百分比，如 0.0520 表示 5.20%）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量
- `unit: 2` — 比率变化
- `unit: 3` — 小数形式的比率变化（百分点变化，如 -0.0015 表示 -0.15 个百分点）
...
```bash
qdm-cmr-cli report store trend --indicator headLossRate
```

返回最近约 30 天的逐日头部门店不良率数据（current）和同比对照数据（compare）：
...
```json
{
  "rows": [
    {
      "period": "2026/05/24",
      "current": 0.0520,       // 当前周期值（需×100）
      "compare": 0.0535        // 同比对照值（去年同期）
    }
  ]
}
```
...
```bash
qdm-cmr-cli report store area --indicator headLossRate
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的头部门店不良率排名（含同比、环比）：
...
1. **valueUnit=3**：头部门店不良率的 valueUnit 为 3（小数比率），需 ×100 转为百分比显示。如 `0.0520` 表示 5.20%。
2. **同比/环比 unit=3**：yoy/mom 中的 value 也是小数形式，表示百分点的变化。
3. **反向指标**：不良率为反向指标，越低越好。头部门店通常不良率较低，若头部门店不良率上升需关注核心门店质量。
