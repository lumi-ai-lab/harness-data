...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取门店面积（`storeArea`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code storeArea --full
```

返回字段：`indicatorsName`（门店面积）、`businessDefinition`（门店营业面积（平方米））、`statisticalLogic`（统计周期内门店实际营业面积）、`indicatorBiz`（门店管理）。
...
```bash
qdm-cmr-cli report store indicators --indicator storeArea --display-mode yoyMom
```
...
- `valueUnit: 2` -- 百分比/比率/金额（直接用值，如 56.93 表示门店面积为 56.93 平方米）
...
```bash
qdm-cmr-cli report store trend --indicator storeArea
```

返回最近约 30 天的逐日门店面积数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator storeArea
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的门店面积排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: 0` 表示箭头向上但无变化。
...
6. 门店面积为坪效（`areaEffective`）的下钻叶子指标，无需获取品类表现数据。