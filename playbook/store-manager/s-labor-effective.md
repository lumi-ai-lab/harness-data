...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取人效（`laborEffective`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code laborEffective --full
```

返回字段：`indicatorsName`（人效）、`businessDefinition`（门店单人均净利润贡献，反映门店人力投入的产出效率）、`statisticalLogic`（门店净利润 / 门店人数）、`indicatorBiz`（门店运营）。
...
```bash
qdm-cmr-cli report store indicators --indicator laborEffective --display-mode yoyMom
```
...
- `valueUnit: 2` — 百分比/比率/金额（直接用值，如 3673.58 表示人效值为 3673.58）
- `valueUnit: 3` — 小数形式的比率（需 x100 转为百分比，如 0.2646 表示 26.46%）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量（如 `-206.68` 表示环比减少 206.68）
...
```bash
qdm-cmr-cli report store trend --indicator laborEffective
```

返回最近约 30 天的逐日人效数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator laborEffective
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的人效排名（含同比、环比）：
...
1. **valueUnit: 2**：表示金额/比率类值，直接使用。`value: 3673.58` 即为人效值 3673.58。
2. **同比/环比 unit: 1**：表示绝对变化量。如 `mom.value: -206.68` 表示环比减少 206.68；`yoy.value: 936.12` 表示同比增加 936.12。
3. **阈值**：阈值符号 `GE`，`compareValue1/compareValue2` 均为 2400，即人效应 >= 2400 方为达标。当前值 3673.58 已达标。
4. 同比（yoy）= 与去年同期对比的变化率；环比（mom）= 与上一个周期对比的变化率。
5. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -206.68` 表示箭头向上但值为负数（即环比下降）。
6. 人效 = 门店净利润 / 门店人数，是门店净利润的下钻分解指标，反映人力投入的产出效率。
