...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取坪效（`areaEffective`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code areaEffective --full
```

返回字段：`indicatorsName`（坪效）、`businessDefinition`（门店每平方米面积的销售额产出）、`statisticalLogic`（门店销售额 / 门店面积）、`indicatorBiz`（门店管理）。
...
```bash
qdm-cmr-cli report store indicators --indicator areaEffective --display-mode yoyMom
```
...
- `valueUnit: 2` -- 百分比/比率/金额（直接用值，如 339.32 表示坪效为 339.32）
...
```bash
qdm-cmr-cli report store trend --indicator areaEffective
```

返回最近约 30 天的逐日坪效数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator areaEffective
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的坪效排名（含同比、环比）：
...
6. 坪效的下钻子指标为门店面积（`storeArea`），可通过同名子命令获取下钻数据。