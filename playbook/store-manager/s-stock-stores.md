...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取存量门店数（`stockStores`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code stockStores --full
```

返回字段：`indicatorsName`（存量门店数）、`businessDefinition`（当前系统中存在的门店总数）、`statisticalLogic`（统计系统中所有门店的数量）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator stockStores --display-mode yoyMom
```
...
- `valueUnit: 1` — 整数值（如门店数，2909 表示 2909 家门店）
- `valueUnit: 2` — 百分比/比率/金额（直接用值）
- `valueUnit: 3` — 小数形式的比率（需 ×100 转为百分比）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量（如 `+5` 表示同比增加 5 家门店）
...
```bash
qdm-cmr-cli report store trend --indicator stockStores
```

返回最近约 30 天的逐日存量门店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator stockStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的存量门店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: 5` 表示箭头向上、同比增加 5 家门店。
