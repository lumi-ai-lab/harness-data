...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取门店净利润（`netProfit`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code netProfit --full
```

返回字段：`indicatorsName`（门店净利润）、`businessDefinition`（门店整体盈利水平）、`statisticalLogic`（门店总收入减去门店总成本后的净利润）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator netProfit --display-mode yoyMom
```
...
- `valueUnit: 2` — 百分比/比率/金额（直接用值，如 667.53 表示门店净利润为 667.53 万元）
...
```bash
qdm-cmr-cli report store trend --indicator netProfit
```

返回最近约 30 天的逐日门店净利润数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator netProfit
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的门店净利润排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: 0` 表示箭头向上但同比持平。
...
6. 门店净利润作为一级核心指标（`showTable`），需在核心指标总览中以表格形式展示，并与营业门店数并列呈现。