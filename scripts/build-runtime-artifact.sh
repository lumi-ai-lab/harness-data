...
copy_tree "$ROOT_DIR/.agents/openclaw" "$OUTPUT_DIR/agents/openclaw"
copy_tree "$ROOT_DIR/.agents/hermes" "$OUTPUT_DIR/agents/hermes"
...
node "$ROOT_DIR/plugins/qdm-html-report/scripts/bundle-dist.mjs"
...
for required in \
  ...
  "$OUTPUT_DIR/plugins/qdm-html-report/.codex-plugin/plugin.json" \
  "$OUTPUT_DIR/plugins/qdm-html-report/.mcp.json" \
  "$OUTPUT_DIR/plugins/qdm-html-report/mcp/server.mjs" \
  "$OUTPUT_DIR/plugins/qdm-html-report/skills/html-report/SKILL.md"; do
  [[ -f "$required" ]] || die "runtime artifact is missing required file: $required"
done

node -e 'const fs = require("fs"); const hooks = JSON.parse(fs.readFileSync(process.argv[1], "utf8")).hooks; for (const event of ["UserPromptSubmit", "PreToolUse", "PostToolUse"]) { if (!hooks[event]?.length) throw new Error("missing Codex hook: " + event); }' "$OUTPUT_DIR/agents/codex/hooks.json"
...
node "$ROOT_DIR/scripts/build-plugin-manifest.mjs" \
  --artifact-root "$OUTPUT_DIR/plugins/qdm-html-report" \
  --host codex \
  --resource-mode external
node "$ROOT_DIR/scripts/verify-artifact.mjs" "$OUTPUT_DIR" --kind runtime

if [[ "$RUN_SELF_TEST" == "1" ]]; then
  (cd "$OUTPUT_DIR" && node plugins/qdm-html-report/mcp/server.mjs --self-test)
fi
