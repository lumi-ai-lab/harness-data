...
class DomainConfig:
    ...
    manifest: Path
    playbook_dir: Path
    default_sample_playbooks: tuple[str, ...]
    company_week_period: bool = False
...
DOMAINS = {
    "business": DomainConfig(
        name="business",
        report="business",
        manifest=ROOT / "scripts" / "cmr_business_playbook_metrics.json",
        playbook_dir=ROOT / "wikis" / "playbooks" / "cmr" / "business",
        default_sample_playbooks=("s-sale-amt.md", "s-cust-num.md", "s-cust-penetration-rate.md", "s-bf19-cust-num.md"),
    ),
    "store-manager": DomainConfig(
        name="store-manager",
        report="store",
        manifest=ROOT / "scripts" / "cmr_store_manager_playbook_metrics.json",
        playbook_dir=ROOT / "wikis" / "playbooks" / "cmr" / "store-manager",
        default_sample_playbooks=("s-close-stores.md", "s-labor-effective.md", "s-net-profit.md"),
    ),
    "member": DomainConfig(
        name="member",
        report="user",
        manifest=ROOT / "scripts" / "cmr_member_playbook_metrics.json",
        playbook_dir=ROOT / "wikis" / "playbooks" / "cmr" / "member",
        default_sample_playbooks=("s-member-num.md", "s-active-member-num.md", "s-buy-member-num.md"),
    ),
    "financial": DomainConfig(
        name="financial",
        report="company",
        manifest=ROOT / "scripts" / "cmr_financial_playbook_metrics.json",
        playbook_dir=ROOT / "wikis" / "playbooks" / "cmr" / "financial",
        default_sample_playbooks=(),
        company_week_period=True,
    ),
}
...
def file_name_for_code(code: str) -> str:
    return f"s-{slug_kebab(code)}.md"


def qdm_week_for_date(day: dt.date) -> str:
    # QDM financial reports only accept week/month. For smoke tests, resolve
    # relative day prompts to the business week containing that day.
    cmd = 'source config/qdm-cli-paths.env && "$QDM_CMR_CLI" search weeks --page-size 20'
    proc = subprocess.run(["bash", "-lc", cmd], cwd=ROOT, text=True, capture_output=True, check=False)
    if proc.returncode == 0:
        try:
            payload = json.loads(proc.stdout)
            for item in payload.get("items", []):
                start = dt.date.fromisoformat(item["startDate"])
                end = dt.date.fromisoformat(item["endDate"])
                if start <= day <= end:
                    return str(item["value"])
        except Exception:
            pass
    return f"{day.isocalendar().year}-{day.isocalendar().week:02d}"


def load_metrics(config: DomainConfig) -> dict[str, SmokeMetric]:
    if not config.manifest.exists():
        raise FileNotFoundError(f"manifest not found: {config.manifest}")
    metrics = json.loads(config.manifest.read_text(encoding="utf-8"))
    result: dict[str, SmokeMetric] = {}
    for item in metrics:
        file_name = file_name_for_code(item["code"])
        result[file_name] = SmokeMetric(
            label=item["label"],
            code=item["code"],
            query_strategy=item.get("query_strategy", "direct_indicator_name"),
            cli_indicator_name=item.get("cli_indicator_name", item["label"]),
        )
    return result
...
def command_has_indicator_name(command: str, indicator_name: str) -> bool:
    pattern = r"--indicator-name(?:=|\s+)(['\"]?)" + re.escape(indicator_name) + r"\1(?:\s|$)"
...
def classify(
    *,
    ...
    target_selected = f"playbooks/cmr/{config.name}/{playbook_file}"
    target_wiki = f"wikis/{target_selected}"
    ...
    if metric.query_strategy == "report_all_filter_code":
        dupont_current = [
            ...
            if "dupont" in cmd
            and f"--report {config.report}" in cmd
            and f"{period_flag} {period_value}" in cmd
            and "--indicator-name" not in cmd
        ]
    else:
        dupont_current = [
            cmd
            ...
            if "dupont" in cmd
            and f"--report {config.report}" in cmd
            and command_has_indicator_name(cmd, metric.cli_indicator_name)
            and f"{period_flag} {period_value}" in cmd
        ]
    final = final_answer(events)
    if metric.query_strategy == "report_all_filter_code":
        answered_with_evidence = bool(dupont_current and final and (metric.code in final or metric.label in final))
    else:
    ...
    non_period_cli_commands = cli_commands
    if config.company_week_period:
        search_commands = [cmd for cmd in search_commands if "search weeks" not in cmd]
        non_period_cli_commands = [cmd for cmd in cli_commands if "search weeks" not in cmd]
    ...
    fail_commands = [
        cmd
        for cmd in cli_commands
        if re.search(r"\barea\b|\btrend\b|report\s+\w+\s+category|report\s+\w+\s+overview|legacy", cmd)
    ]
    if not dupont_current:
    ...
    elif search_commands or duplicate_dupont or full_commands or len(non_period_cli_commands) > 1:
        shortest_status = "WARN"
        if len(non_period_cli_commands) > 1 and not shortest_reasons:
...
def run_one(
    ...
    with stream_path.open("w", encoding="utf-8") as stdout_file:
        try:
            ...
            timed_out = True
            proc_returncode = 124
            proc_stderr = f"timeout after {per_sample_timeout}s: {exc.stderr or ''}"
    ...
    if timed_out:
        ...
        shortest_status = "FAIL"
        optimization = "需要优化"
        reasons.append(f"claude timeout after {per_sample_timeout}s")
    elif proc_returncode != 0:
        playbook_status = "FAIL"
        shortest_status = "FAIL"
        optimization = "需要优化"
        reasons.append(f"claude exit={proc_returncode}: {proc_stderr.strip()[:300]}")

    return {
        "domain": config.name,
        "label": metric.label,
        "playbook_file": playbook_file,
        "target_playbook": f"playbooks/cmr/{config.name}/{playbook_file}",
        "prompt": prompt,
...
def write_summary(config: DomainConfig, results: list[dict[str, Any]], out_dir: Path) -> None:
    lines: list[str] = []
    lines.append(f"# CMR {config.name} Playbook Smoke 测试报告")
    lines.append("")
    lines.append(f"- 运行目录：`{out_dir}`")
    lines.append(f"- domain：`{config.name}`")
    lines.append(f"- report alias：`{config.report}`")
    ...
    if fail_warn:
        ...
        for item in fail_warn:
            lines.append(f"- `{item['playbook_file']}`：{markdown_escape('；'.join(dict.fromkeys(item['reasons'])))}")
    lines.append("")
    lines.append("## 人工复核证据")
    for item in results:
        lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CMR single-playbook smoke tests through Claude Code.")
    ...
    parser.add_argument("--output-root", type=Path, default=ROOT / "runs" / "cmr-playbook-smoke")
    parser.add_argument("--playbook", action="append", help="Sample playbook file name. Can be passed multiple times.")
    parser.add_argument("--all", action="store_true", help="Run all playbooks in the domain manifest.")
    parser.add_argument("--per-sample-timeout", type=int, default=180, help="Timeout seconds for each isolated Claude run.")
...
def main() -> int:
    ...
    if config.company_week_period:
        period_flag = "--week"
        period_value = qdm_week_for_date(current_date - dt.timedelta(days=1))

    file_to_metric = load_metrics(config)
    if args.all and args.playbook:
        raise SystemExit("--all cannot be combined with --playbook")
    sample_files = list(file_to_metric) if args.all else (args.playbook or list(config.default_sample_playbooks))
    if not sample_files:
        raise SystemExit(f"no default sample playbooks configured for domain {config.name}; pass --playbook or --all")
    missing = [file_name for file_name in sample_files if file_name not in file_to_metric]
    if missing:
        raise SystemExit(f"missing playbook label mapping: {', '.join(missing)}")
    missing_files = [file_name for file_name in sample_files if not (config.playbook_dir / file_name).exists()]
    if missing_files:
        raise SystemExit(f"missing playbook files in {config.playbook_dir}: {', '.join(missing_files)}")

    timestamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    out_dir = args.output_root / config.name / timestamp
    out_dir.mkdir(parents=True, exist_ok=False)

    results: list[dict[str, Any]] = []
    for file_name in sample_files:
        metric = file_to_metric[file_name]
        print(f"running {config.name}/{file_name} ({metric.label})", file=sys.stderr, flush=True)
        results.append(
            run_one(
                config,
                metric,
                ...
                period_flag,
                period_value,
                args.per_sample_timeout,
            )
        )
        write_summary(config, results, out_dir)
