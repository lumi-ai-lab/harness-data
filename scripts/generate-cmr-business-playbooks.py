#!/usr/bin/env python3
"""Generate CMR business single-metric playbooks from a JSON manifest."""
...
DEFAULT_MANIFEST = ROOT / "scripts" / "cmr_business_playbook_metrics.json"
DEFAULT_TEMPLATE = ROOT / "scripts" / "templates" / "cmr_business_single_playbook.md.j2"
SPEC_DIR = ROOT / "wikis" / "spec" / "cmr" / "business"
...
def render_playbook(template: str, metric: Metric) -> str:
    ...
    values = {
        "frontmatter_name": f"playbook_cmr_business_s_{slug_snake(metric.code)}",
        "label": metric.label,
        "code": metric.code,
