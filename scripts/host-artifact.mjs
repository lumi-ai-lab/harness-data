...
// Internal adapter test builder. Host artifacts are not a current user release
// channel; the Codex Plugin is released directly from the main repository.
...
export function buildHostArtifact({ host, artifactRoot, version, selfTest = true, repoRoot = REPO_ROOT } = {}) {
  ...
  stageAdapter(root, target, spec, pluginVersion);
...
function stageCommonFiles(repoRoot, target) {
  ...
  writeFileSync(
    path.join(target, "bin", "data-harness-cli"),
    "#!/usr/bin/env node\n\nimport { main } from \"./../vendor/data-harness-cli/src/main.js\";\n\nawait main();\n",
    { mode: 0o755 },
  );
  mkdirSync(path.join(target, "scripts"), { recursive: true });
  writeFileSync(
    path.join(target, "scripts", "data-harness-cli"),
    "#!/usr/bin/env node\n\nimport { main } from \"./../vendor/data-harness-cli/src/main.js\";\n\nawait main();\n",
    { mode: 0o755 },
  );
  copyTree(path.join(repoRoot, "plugins", "harness-data", "skills", "html-report"), path.join(target, "skills", "html-report"));
...
function stageAdapter(repoRoot, target, spec, pluginVersion) {
  ...
  if (spec.host !== "pi") {
    ...
    if (spec.host === "claude" || spec.host === "codex") {
      const manifestPath = path.join(target, spec.adapterManifest);
      const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
      writeJSON(manifestPath, { ...manifest, version: pluginVersion });
    }
    if (spec.host === "codex") {
      const dist = path.join(adapter, "dist");
      rmSync(dist, { recursive: true, force: true });
      for (const name of ["data-harness-cli", "html-report-kernel", "harness-runtime-node"]) {
        copyTree(path.join(repoRoot, "packages", name), path.join(dist, name));
      }
      copyTree(path.join(repoRoot, "npm"), path.join(dist, "harness-data-installer"));
      copyFile(path.join(repoRoot, "bootstrap", "cli-manifest.json"), path.join(adapter, "bootstrap", "cli-manifest.json"));
    }
    return;
  }
