...
截至 `0.0.54`，停止支持日期尚未对外公布；`0.0.55-test` 不计入正式兼容窗口。未完成正式 `0.0.55` 发布、完整周期观察和用户可见 deprecation 前，`install --dir` 继续受支持。日常排障和发布门槛见 [`support-runbook.md`](support-runbook.md)。
