# v0.0.54 历史发布验收记录（2026-08-29）

> 历史归档。本文记录旧发布链路的事实，不是当前 Harness Data 用户发布契约。当前契约以 `README.md`、`docs/codex-plugin-layout.md` 和 Codex Plugin 验证 workflow 为准。
...
- Codex 真实升级/回滚、宿主原生 secret provider 和桌面 reload 仍属于 Phase 5 后续实机验收；仓库已补充 update 全流程回滚、CODEX_HOME 优先级和继承 FD secret transport 自动化覆盖。
