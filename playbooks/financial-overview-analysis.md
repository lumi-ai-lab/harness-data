...
- PostToolUse 阶段可记录指定 report 的必需取数模块，并在 signal 后二阶段注入 template。
...
- 已新增 `spec/financial-report.md`，明确 EBITDA、收入、毛利、费用指标的章节归属和禁放规则。
- 已新增测试覆盖：用户 prompt 注入、company indicators/tree/table 模块记录、signal 后财务模板注入、未取数 signal 的缺失模块提示。
