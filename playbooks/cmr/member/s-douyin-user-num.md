# 抖音用户数指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取抖音用户数（`douyinUserNum`）指标的详情和数据。用户报表使用 `report user` 子命令。
>
...
```bash
qdm-cmr-cli indicator detail --code douyinUserNum --full
```

返回字段：`indicatorsName`（抖音用户数）、`businessDefinition`（统计周期内，最后一天的抖音钱大妈QDM的粉丝数量）、`statisticalLogic`（抖音公众号粉丝数量）。
...
```bash
qdm-cmr-cli report user indicators --indicator douyinUserNum --display-mode yoyMom
```
...
```bash
qdm-cmr-cli report user trend --indicator douyinUserNum
```
...
```bash
qdm-cmr-cli report user area --indicator douyinUserNum
```
...
3. 抖音用户数 `threshold` 为 null，无预设阈值。
4. 用户报表**不支持品类过滤**，不要传递 `--category-type` 或 `--category` 参数。
5. 抖音用户数是叶子指标，无下级子指标，不支持下钻分析。
6. 父指标为可触达用户数（`reachMemberNum`）。
