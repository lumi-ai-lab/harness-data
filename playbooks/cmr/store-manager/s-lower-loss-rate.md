# 中下门店不良率指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取中下门店不良率（`lowerLossRate`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。
> 品类口径固定为全品类，不支持品类下钻。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code lowerLossRate --full
```

返回字段：`indicatorsName`（中下门店不良率）、`businessDefinition`（中下门店中不良门店的占比）、`indicatorBiz`（门店盈利与运营效率）。
...
```bash
qdm-cmr-cli report store indicators --indicator lowerLossRate --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "lowerLossRate",
  "indicatorName": "中下门店不良率",
  "value": 0.2150,            // 当前值（valueUnit=3，需×100=21.50%）
  "valueUnit": 3,              // 单位类型（3=小数形式的比率，需×100转为百分比）
  "mom": {
    "value": -0.0025,         // 环比变动（-0.25个百分点）
    "status": "down",
    "unit": 3
  },
  "yoy": {
    "value": 0.0040,          // 同比变动（+0.40个百分点）
    "status": "up",
    "unit": 3
  }
}
...
- `valueUnit: 3` — 小数形式的比率（需 ×100 转为百分比，如 0.2150 表示 21.50%）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量
- `unit: 2` — 比率变化
- `unit: 3` — 小数形式的比率变化（百分点变化，如 0.0040 表示 +0.40 个百分点）
...
```bash
qdm-cmr-cli report store trend --indicator lowerLossRate
```

返回最近约 30 天的逐日中下门店不良率数据（current）和同比对照数据（compare）：
...
```json
{
  "rows": [
    {
      "period": "2026/05/24",
      "current": 0.2150,       // 当前周期值（需×100）
      "compare": 0.2110        // 同比对照值（去年同期）
    }
  ]
}
```
...
```bash
qdm-cmr-cli report store area --indicator lowerLossRate
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的中下门店不良率排名（含同比、环比）：
...
1. **valueUnit=3**：中下门店不良率的 valueUnit 为 3（小数比率），需 ×100 转为百分比显示。如 `0.2150` 表示 21.50%。
2. **同比/环比 unit=3**：yoy/mom 中的 value 也是小数形式，表示百分点的变化。
3. **反向指标**：不良率为反向指标，越低越好。中下门店不良率通常显著高于头部和中上梯队，为不良率管控的重点关注梯队。
