...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取员工工资（`storeTotalSalary`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code storeTotalSalary --full
```

返回字段：`indicatorsName`（员工工资）、`businessDefinition`（门店员工工资总额（万元））、`statisticalLogic`（统计周期内门店员工工资合计）、`indicatorBiz`（门店管理）。
...
```bash
qdm-cmr-cli report store indicators --indicator storeTotalSalary --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "storeTotalSalary",
  "indicatorName": "员工工资",
  "value": 970.54,
  "valueUnit": 2,
  "mom": {
...
- `valueUnit: 2` -- 百分比/比率/金额（直接用值，如 970.54 表示员工工资为 970.54 万元）
...
```bash
qdm-cmr-cli report store trend --indicator storeTotalSalary
```

返回最近约 30 天的逐日员工工资数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator storeTotalSalary
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的员工工资排名（含同比、环比）：
