# 待开业门店数指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取待开业门店数（`unopenStores`）指标的值、趋势、区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code unopenStores --full
```

返回字段：`indicatorsName`（待开业门店数）、`businessDefinition`（已批准筹建但尚未正式营业的门店数量）、`statisticalLogic`（统计截止日期当天，所有状态为"待开业"或"筹建中"的门店数量）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator unopenStores --display-mode yoyMom
```
...
```json
{
  "indicatorCode": "unopenStores",
  "indicatorName": "待开业门店数",
  "value": 18,
  "valueUnit": 1,
  "mom": {
    "value": 0,
...
```bash
qdm-cmr-cli report store trend --indicator unopenStores
```

返回最近约 30 天的逐日待开业门店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator unopenStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的待开业门店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -3` 表示箭头向上但数值为 -3（即同比减少 3 家门店）。
