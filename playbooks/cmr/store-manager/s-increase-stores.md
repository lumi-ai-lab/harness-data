# 净增门店数指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取净增门店数（`increaseStores`）指标的值、趋势、区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code increaseStores --full
```

返回字段：`indicatorsName`（净增门店数）、`businessDefinition`（报告期内净增加的门店数量）、`statisticalLogic`（开店数 - 闭店数）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator increaseStores --display-mode yoyMom
```
...
```bash
qdm-cmr-cli report store trend --indicator increaseStores
```

返回最近约 30 天的逐日净增门店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator increaseStores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的净增门店数排名（含同比、环比）：
...
2. 同比/环比中的 `status` 字段（`up`/`down`）表示箭头指向，不代表数值正负。`status: "up"` 且 `value: -10` 表示箭头向上但数值为 -10（即环比减少 10 家门店）。
