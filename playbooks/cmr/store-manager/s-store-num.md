...
> 本手册讲解如何通过 `qdm-cmr-cli` 获取门店人数（`storeNum`）指标的值、趋势和区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code storeNum --full
```

返回字段：`indicatorsName`（门店人数）、`businessDefinition`（门店实际在岗人员数量）、`statisticalLogic`（统计周期内门店平均在岗人员数量）、`indicatorBiz`（门店运营）。
...
```bash
qdm-cmr-cli report store indicators --indicator storeNum --display-mode yoyMom
```
...
- `valueUnit: 2` — 百分比/比率/金额（直接用值，如 5.26 表示门店人数为 5.26，店日均口径）
- `valueUnit: 3` — 小数形式的比率（需 x100 转为百分比，如 0.2646 表示 26.46%）

**同比/环比的 value**：
- `unit: 1` — 绝对变化量（如 `0` 表示无变化）
...
```bash
qdm-cmr-cli report store trend --indicator storeNum
```

返回最近约 30 天的逐日门店人数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator storeNum
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的门店人数排名（含同比、环比）：
...
1. **valueUnit: 2**：表示金额/比率类值，直接使用。`value: 5.26` 即为门店人数 5.26（店日均口径，非整数因平滑计算）。
2. **同比/环比 unit: 1**：表示绝对变化量。`mom.value: 0` 和 `yoy.value: 0` 表示同环比均无变化。
3. **阈值**：阈值符号 `LE`（小于等于），`compareValue1/compareValue2` 均为 5，即门店人数应 <= 5 方为达标。当前值 5.26 > 5，未达标。
4. 门店人数是人效计算的分母（人效 = 门店净利润 / 门店人数），门店人数越低，在净利润不变的情况下人效越高。
