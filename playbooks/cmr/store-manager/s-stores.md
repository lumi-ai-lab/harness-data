# 营业门店数指标取数手册 (Playbook)

> 本手册讲解如何通过 `qdm-cmr-cli` 获取营业门店数（`stores`）指标的值、趋势、区域表现数据，以及如何添加过滤条件。

## 一、获取指标详情

```bash
qdm-cmr-cli indicator detail --code stores --full
```

返回字段：`indicatorsName`（营业门店数）、`businessDefinition`（当前处于正常营业状态的门店总数）、`statisticalLogic`（统计截止日期当天，所有状态为"营业中"的门店数量）、`indicatorBiz`（门店管理环节）。
...
```bash
qdm-cmr-cli report store indicators --indicator stores --display-mode yoyMom
```
...
```bash
qdm-cmr-cli report store trend --indicator stores
```

返回最近约 30 天的逐日营业门店数数据（current）和同比对照数据（compare）：
...
```bash
qdm-cmr-cli report store area --indicator stores
```

默认按**管理区域**（`manageAreaId`）维度分组，返回全国各区域的营业门店数排名（含同比、环比）：
