...
- Do not invent or pass `--data-auth`, `--auth-blob`, or `--auth-json`; the Codex PreToolUse authz hook injects authorization for `qdm-metric-cli analysis validate|preview|execute|total`, `dim values`, `tag list`, and `auth describe`.
