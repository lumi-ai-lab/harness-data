...
- When `authzMode=on`, obtain the account data scope (`sapArea2Id`, `dcSapArea2Id`, and `categoryLevel1Id`) from `qdm-metric-cli auth describe` before the first user-facing answer based on metric results. Cache it for the current session, disclose it whenever reporting permission-scoped numbers, and never describe those numbers as unrestricted totals.
