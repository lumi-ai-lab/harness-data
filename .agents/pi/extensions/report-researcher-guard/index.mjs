...
export const TYPED_RESEARCHER_NEW_QUERY_SYSTEM_PROMPT = [
  ...
  "1) Read the assigned absolute result.json exactly once. Select cards[].id === fromCardId and deep-copy its requestBody as the immutable baseline.",
  ...
  "3) Change only the indicator/dimension/date-comparison/filter-scope/metric-definition fields explicitly authorized by evidenceGap. Preserve all other baseline fields; orderBy, currPage, pageSize, chartType and unknown fields are never a material delta.",
  "4) Write exactly $SESSION/data/explore/<taskId>.payload.json once. Then run exactly once: node .agents/pi/skills/html-report/scripts/fetch-explore.mjs --result <ABS_RESULT_JSON> --task-id <TASK_ID> --payload-file <ABS_PAYLOAD_JSON> --goal <EXACT_TASK_GOAL> --from-card-id <EXACT_FROM_CARD_ID>.",
