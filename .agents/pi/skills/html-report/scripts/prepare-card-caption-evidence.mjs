...
import { extractRows } from "./assemble-report.mjs";
import {
  buildCaptionAxis,
  captionPrefixes,
  captionViewId,
...
function rowSlice(row, metric, axis) {
  const sliced = {};
  for (const dim of axis) {
    if (Object.prototype.hasOwnProperty.call(row, dim)) sliced[dim] = row[dim];
  }
  if (Object.prototype.hasOwnProperty.call(row, metric)) sliced[metric] = row[metric];
  for (const suffix of CAPTION_COMPARISON_SUFFIXES) {
    const field = comparisonField(metric, suffix);
    if (Object.prototype.hasOwnProperty.call(row, field)) sliced[field] = row[field];
  }
  return sliced;
}

function prefixKey(row, prefix) {
  return JSON.stringify(prefix.map((dim) => (Object.prototype.hasOwnProperty.call(row, dim) ? row[dim] : null)));
}

function prefixObject(row, prefix) {
  return Object.fromEntries(prefix.map((dim) => [dim, Object.prototype.hasOwnProperty.call(row, dim) ? row[dim] : null]));
}
...
function viewRows(ranked, metric, prefix, axis) {
  return ranked.map((item, rank) => ({
    rank: rank + 1,
    key: prefixObject(item.row, prefix),
    metricValue: item.metricValue,
    row: rowSlice(item.row, metric, axis),
  }));
}

export function buildCaptionEvidence({ cardId, query, rows }) {
    ...
    for (const prefix of captionPrefixes(axis)) {
      ...
      if (top.length) {
        ...
        views[id] = {
          ...
          dimensions: prefix,
          n: CAPTION_N,
          rows: viewRows(top, metric, prefix, axis),
        };
      }
      if (bottom.length) {
        ...
        views[id] = {
          ...
          dimensions: prefix,
          n: CAPTION_N,
          rows: viewRows(bottom, metric, prefix, axis),
        };
      }
    }
  }
