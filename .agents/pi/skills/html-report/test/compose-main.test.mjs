...
test("compose-main renders Chinese headers and dimension-first ordering with column-meta + query", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "html-report-compose-cn-"));
  ...
  await writeFile(join(session, "data", "cards", "c1", "caption.md"), "深圳店销售额 100。\n");
  ...
  const output = await composeMain(session);
  const main = await readFile(output.mainPath, "utf8");
  // date group (日期) first, then store group (城市, 门店), then metric (销售额).
  assert.match(main, /\| 日期 \| 城市 \| 门店 \| 销售额 \|/);
  assert.match(main, /\| 2026-07-01 \| 深圳 \| 深圳店 \| 100 \|/);
});
